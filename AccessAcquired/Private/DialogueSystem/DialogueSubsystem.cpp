...
void UDialogueSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	{
		PrintToScreenError("Launch terminated due to errors. See Output Log for more details.");
	}
}
...
void UDialogueSubsystem::StartInnerVoiceDialogue(const FString& DialogueName)
{
	...
	if (UWidgetMenuStack* MenuStack = AAAHUD::GetMenuStack(this))
	{
		...
		InnerVoiceWidget->Execute_QueueDialogue(InnerVoiceWidget, ReceivedPackagedDialogue);
		InnerVoiceWidget->Execute_StartDialogue(InnerVoiceWidget);

		APlayerController* PlayerController = UGameplayStatics::GetPlayerController(GetWorld(), 0);
		IPlayerControllerInterface::Execute_ActivateUIInput(PlayerController, InnerVoiceWidget);
	}
}
